/* --- Main Ultrasonic Sensor and Bluetooth Master Code for Contact Tracing App.
...
int id = 805191;                        // Arbitrary ID number.

bool diseaseStatus = true;              // Set whether or not the user has the contagious disease with this variable.
...
void dist(int distance)
{
    ...
    if (rn41.writable())
    {
        // Code to send to the slave device:
    ...
    {   
        // Code to read from the slave device:
...
ultrasonic mu(p12, p13, 1, 2, &dist);   // Set the trigger pin to p12 and the echo pin to p13.
...
{   
    ...
    if (diseaseStatus == true)
    {
        string diseaseDiagnosis = "Pos. for disease.";
    } else if (diseaseStatus == false)
    {
        string diseaseDiagnosis = "Neg. for disease.";
    }
